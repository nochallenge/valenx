...
- **Tauri** — optional if we want web-tech for parts of the UI (e.g., the
  docs viewer) — but the core app is pure Rust/egui, no web layer
...
| Language | Why not |
|---|---|
| **JavaScript / TypeScript** | No web frontend. If we need DOM in a limited doc viewer, use Tauri's webview for that one panel only |
