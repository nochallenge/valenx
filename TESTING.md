...
## Comparison: how we USED to test (web-based)

| Old (web) | New (native) |
|---|---|
| `npm run dev` → browser at localhost:5173 | `cargo run -p valenx-app` → native window opens |
| React DevTools in browser | LLDB debugger in IDE |
| Vite hot-reload on save | `cargo watch -x "run -p valenx-app"` (slightly slower) |
| Jest / Vitest for JS tests | `bash scripts/qa.sh` (scoped QA harness) |
| Lighthouse / browser perf tab | `cargo bench` + flamegraphs |
| Playwright / Cypress for E2E | `egui_kittest` snapshots |
| Deploy preview URL | Signed installer artifact from CI |

Everything maps 1:1. The native version is generally faster to iterate
on once you're past the initial dep compilation.
