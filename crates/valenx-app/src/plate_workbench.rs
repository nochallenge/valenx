        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("radius a (m)");
                        ui.add(egui::DragValue::new(&mut s.radius_m).speed(0.01))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("thickness t (m)");
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 5,
            "expected the plate numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every plate DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["radius a (m)", "thickness t (m)", "pressure p (Pa)"] {
            assert!(
