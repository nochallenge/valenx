...
pub fn draw_batterypack_workbench(app: &mut ValenxApp, ctx: &egui::Context) {
    ...
    let close = crate::workbench_chrome::workbench_shell(
        app,
        ctx,
        "valenx_batterypack_workbench",
        "Battery Pack",
        |app, ui| {
            ui.label(
                egui::RichText::new("native series / parallel pack sizing · valenx-batterypack")
                    .weak()
                    .small(),
            );
            ui.separator();
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("nominal voltage (V)");
                        ui.add(egui::DragValue::new(&mut s.cell_voltage_v).speed(0.05))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("capacity (Ah)");
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("series n");
                        ui.add(egui::DragValue::new(&mut s.series).speed(1.0))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("parallel m");
...
/// The agent-bridge **`show_3d{kind:"batterypack"}`** product: the canonical
/// battery pack built as a 3-D solid, paired with the workbench's own
/// `compute()` readout rows, at a fixed 3/4 camera. Registered in
/// [`crate::products_registry`]; the per-tool builder the registry dispatches
/// to. Pure — driven off [`BatteryPackWorkbenchState::default`].
pub(crate) fn batterypack_product() -> crate::WorkspaceProduct {
    let s = BatteryPackWorkbenchState::default();
    let mesh = pack_solid_mesh(&s).expect("canonical pack ⇒ solid builds");
    let loaded = crate::products_registry::loaded_mesh_from(mesh, "<pack>/valenx-batterypack");
    let lines = crate::products_registry::lines_from_readout(
        &compute(&s).expect("canonical pack ⇒ readout computes"),
    );
    let camera = crate::products_registry::camera_for(&loaded.mesh);
    crate::WorkspaceProduct {
        title: "Battery pack".into(),
        lines,
        mesh: Some(loaded),
        vertex_colors: None,
        camera,
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        for caption in ["nominal voltage (V)", "series n", "C-rate (1/h)"] {
            assert!(
