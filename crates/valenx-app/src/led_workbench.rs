        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("supply Vs (V)");
                        ui.add(egui::DragValue::new(&mut s.supply_v).speed(0.1))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("forward Vf (V)");
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 3,
            "expected the led numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every led DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["supply Vs (V)", "forward Vf (V)", "current I (A)"] {
            assert!(
