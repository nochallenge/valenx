        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("Young's modulus E (MPa)");
                        ui.add(
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("Poisson's ratio ν");
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 5,
            "expected the strainrosette numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every strainrosette DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["ε₀ (0°)", "ε₄₅ (45°)", "ε₉₀ (90°)"] {
            assert!(
