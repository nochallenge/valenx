        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    // reader / AI driver otherwise). Each stage's in/out pair
                    // shares the one "teeth in / out" caption.
                    ui.horizontal(|ui| {
                        let l = ui.label("teeth in / out");
...
mod headless_ui_tests {
    ...
    fn workbench_draws_a_result_and_error_without_panic() {
        ...
        app.gearbox.error = Some("invalid gear-train parameters".to_string());
        draw_workbench(&mut app);
    }

    #[test]
    fn numeric_controls_are_named_and_associated() {
        // The stage tooth counts + mesh/input DragValues are SpinButtons; each
        // must be `labelled_by` its caption (egui clears a DragValue's own Name),
        // so an AI / screen reader can find the control by the caption text. Each
        // stage's in/out pair shares the one "teeth in / out" caption.
        let mut app = ValenxApp::default();
        app.show_gearbox_workbench = true;
        ...
        let spin_buttons: Vec<&Node> = nodes
            ...
            .collect();
        // stage1 in/out, stage2 in/out, efficiency, input speed, input torque.
        assert!(
            spin_buttons.len() >= 7,
            "expected the gearbox numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every gearbox DragValue must be labelled_by its caption (AI-drivable name)"
        );
