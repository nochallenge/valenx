...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        // The shaft + load + material DragValues are SpinButtons; each must be
        ...
        let spin_buttons: Vec<&Node> = nodes
            ...
            .collect();
        // diameter, bending moment, torque, allowable shear.
        assert!(
            spin_buttons.len() >= 4,
            "expected the shaft numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every shaft DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["diameter d (mm)", "torque T (N·m)", "allowable shear (MPa)"] {
            assert!(
