        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("dry-bulb (°C)");
                        ui.add(egui::DragValue::new(&mut s.dry_bulb_c).speed(0.5))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("relative humidity (%)");
                        ui.add(
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 3,
            "expected the psychrometrics numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every psychrometrics DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["dry-bulb (°C)", "relative humidity (%)", "pressure (kPa)"] {
            assert!(
