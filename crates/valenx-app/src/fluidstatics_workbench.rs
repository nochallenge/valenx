...
/// The agent-bridge **`show_3d{kind:"fluidstatics"}`** product: the canonical
/// hydrostatic tank built as a 3-D solid, paired with the workbench's own
/// `compute()` readout rows, at a fixed 3/4 camera. Registered in
/// [`crate::products_registry`]; the per-tool builder the registry dispatches
/// to. Pure — driven off [`FluidStaticsWorkbenchState::default`].
pub(crate) fn fluidstatics_product() -> crate::WorkspaceProduct {
    let s = FluidStaticsWorkbenchState::default();
    let mesh = tank_solid_mesh(&s).expect("canonical fluid statics ⇒ tank solid builds");
    let loaded = crate::products_registry::loaded_mesh_from(mesh, "<fluidstatics>/valenx-tank");
    let lines = crate::products_registry::lines_from_readout(
        &compute(&s).expect("canonical fluid statics ⇒ readout computes"),
    );
    let camera = crate::products_registry::camera_for(&loaded.mesh);
    crate::WorkspaceProduct {
        title: "Fluid statics (tank hydrostatics)".into(),
        lines,
        mesh: Some(loaded),
        vertex_colors: None,
        camera,
