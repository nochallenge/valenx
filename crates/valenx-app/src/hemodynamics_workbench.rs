        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let lbl = ui.label("start P0 (mmHg)");
                        ui.add(egui::DragValue::new(&mut s.windkessel_p0_mmhg).speed(1.0))
                            .labelled_by(lbl.id)
                            .on_hover_text("start P0 (mmHg)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("resistance R (Pa·s/m³)");
                        ui.add(egui::DragValue::new(&mut s.windkessel_r).speed(1.0e6))
                            .labelled_by(lbl.id)
                            .on_hover_text("resistance R (Pa·s/m³)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("compliance C (m³/Pa)");
                        ui.add(egui::DragValue::new(&mut s.windkessel_c).speed(1.0e-10))
                            .labelled_by(lbl.id)
                            .on_hover_text("compliance C (m³/Pa)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("elapsed t (s)");
                        ui.add(egui::DragValue::new(&mut s.decay_time_s).speed(0.05))
                            .labelled_by(lbl.id)
                            .on_hover_text("elapsed t (s)");
                    });
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        // Every heart / vessel / Windkessel DragValue is a SpinButton; each
        ...
        // text. All ten are visible in the default state (the vessel-scale
        // toggle is a radio, not a conditional field).
        let mut app = ValenxApp::default();
        app.show_hemodynamics_workbench = true;
        ...
        let spin_buttons: Vec<&Node> = nodes
            ...
            .collect();
        // heart rate, stroke volume, radius, length, viscosity, pressure drop,
        // start P0, resistance R, compliance C, elapsed t.
        assert!(
            spin_buttons.len() >= 10,
            "expected the hemodynamics numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every hemodynamics DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["heart rate (bpm)", "radius (mm)", "resistance R (Pa·s/m³)"] {
            assert!(
