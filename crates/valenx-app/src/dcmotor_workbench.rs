        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("supply V (V)");
                        ui.add(egui::DragValue::new(&mut s.supply_v).speed(0.5))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("shaft ω (rad/s)");
...
mod headless_ui_tests {
    ...
    fn workbench_draws_a_result_and_error_without_panic() {
        ...
        app.dcmotor.error = Some("invalid motor parameters".to_string());
        draw_workbench(&mut app);
    }

    #[test]
    fn numeric_controls_are_named_and_associated() {
        // The motor-constant + operating-point DragValues are SpinButtons; each
        ...
        let spin_buttons: Vec<&Node> = nodes
            ...
            .collect();
        // resistance, Ke, Kt, supply V, shaft ω.
        assert!(
            spin_buttons.len() >= 5,
            "expected the motor numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every motor DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["resistance R (Ω)", "supply V (V)", "shaft ω (rad/s)"] {
            assert!(
