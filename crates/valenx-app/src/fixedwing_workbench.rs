        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let lbl = ui.label("wing area S (m²)");
                        ui.add(egui::DragValue::new(&mut s.wing_area_m2).speed(0.5))
                            .labelled_by(lbl.id)
                            .on_hover_text("wing area S (m²)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("all-up mass (kg)");
                        ui.add(egui::DragValue::new(&mut s.mass_kg).speed(10.0))
                            .labelled_by(lbl.id)
                            .on_hover_text("all-up mass (kg)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("aspect ratio AR");
                        ui.add(egui::DragValue::new(&mut s.aspect_ratio).speed(0.1))
                            .labelled_by(lbl.id)
                            .on_hover_text("aspect ratio AR");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("CLmax");
                    ...
                    ui.horizontal(|ui| {
                        let lbl = ui.label("cruise v (m/s)");
                        ui.add(egui::DragValue::new(&mut s.cruise_speed_m_s).speed(1.0))
                            .labelled_by(lbl.id)
                            .on_hover_text("cruise v (m/s)");
                    });
                    ui.horizontal(|ui| {
                        let lbl = ui.label("glide altitude (m)");
                        ui.add(egui::DragValue::new(&mut s.glide_altitude_m).speed(50.0))
                            .labelled_by(lbl.id)
                            .on_hover_text("glide altitude (m)");
                    });
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        // All nine aircraft DragValues are SpinButtons; each must be
        ...
        let spin_buttons: Vec<&Node> = nodes
            ...
            .collect();
        // wing area, all-up mass, aspect ratio, CLmax, CD0, Oswald e, air ρ,
        // cruise v, glide altitude.
        assert!(
            spin_buttons.len() >= 9,
            "expected the aircraft numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every aircraft DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["wing area S (m²)", "all-up mass (kg)", "glide altitude (m)"] {
            assert!(
