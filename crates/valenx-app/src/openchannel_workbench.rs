        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("Manning n");
                        ui.add(egui::DragValue::new(&mut s.manning_n).speed(0.001))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("bed slope S (m/m)");
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 4,
            "expected the openchannel numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every openchannel DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["bottom width b (m)", "flow depth y (m)", "Manning n"] {
            assert!(
