        ...
        |app, ui| {
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("gauge factor GF");
                        ui.add(egui::DragValue::new(&mut s.gauge_factor).speed(0.01))
                            .labelled_by(l.id);
                    });
                    ui.horizontal(|ui| {
                        let l = ui.label("nominal R (Ω)");
                    ...
                    ui.horizontal(|ui| {
                        let l = ui.label("excitation Vin (V)");
...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        assert!(
            spin_buttons.len() >= 5,
            "expected the straingauge numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every straingauge DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["gauge factor GF", "nominal R (Ω)", "strain (µε)"] {
            assert!(
