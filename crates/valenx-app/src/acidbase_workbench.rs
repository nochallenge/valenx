...
mod headless_ui_tests {
    ...
    fn numeric_controls_are_named_and_associated() {
        ...
        // a buffer, which renders three fields: pKa, [HA], [A⁻].
        let mut app = ValenxApp::default();
        app.show_acidbase_workbench = true;
        ...
        assert!(
            spin_buttons.len() >= 3,
            "expected the buffer numeric controls as spin buttons, got {}",
            spin_buttons.len()
        );
        assert!(
            spin_buttons.iter().all(|n| !n.labelled_by().is_empty()),
            "every acid-base DragValue must be labelled_by its caption (AI-drivable name)"
        );

        for caption in ["pKa", "[HA] acid (mol/L)", "[A⁻] base (mol/L)"] {
            assert!(
