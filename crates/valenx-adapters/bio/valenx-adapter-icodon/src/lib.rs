//! # valenx-adapter-icodon
//!
//! Adapter for [iCodon](https://github.com/santiago1234/iCodon) — the
//! Vejnar lab's R package for codon-level mRNA stability prediction.
//! iCodon trains species-specific regressors over codon frequencies
//! to score mRNA decay rates, exposing a `predict_stability()`
//! function that returns per-transcript stability for human, mouse,
//! frog, fish, and several other reference organisms.
//!
//! **Phase 43 — Rscript subprocess wrapper for user-provided R
//! scripts.** iCodon is an R package (not a CLI), so the adapter
//! itself doesn't generate R; the user authors a `predict.R`
//! referenced from `[bio.icodon].script` in `case.toml` that does
//! `library(iCodon)` and the actual prediction work. `prepare()`
//! stages the script (and an optional input FASTA) into the workdir,
...
//! `<basename>*.tsv` (stability tables), `<basename>*.rds` (R
//! objects), and `<basename>*.png` (plots), plus any `*.log` files
//! Rscript emits.
...
use crate::case_input::IcodonInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(IcodonAdapter::new())
}

pub struct IcodonAdapter;
...
impl Default for IcodonAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "icodon";
/// We probe `Rscript` (the non-interactive R driver). iCodon itself
/// is an R package — confirming it's installed requires running R,
/// which we deliberately don't do at probe time. The script's
/// `library(iCodon)` call surfaces a missing-package error at run
/// time with R's own (clear) diagnostic.
const RSCRIPT_BINARIES: &[&str] = &["Rscript"];

impl Adapter for IcodonAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "iCodon",
            // iCodon's GitHub releases haven't published versioned
            // tags consistently; the package self-reports 1.x. Upper
            // bound 2.0 reserves room for a future major.
            ...
            docs_url: "https://github.com/santiago1234/iCodon",
            homepage_url: "https://github.com/santiago1234/iCodon",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(RSCRIPT_BINARIES) {
            Some(binary_path) => Ok(ProbeReport {
                ok: true,
                // We don't run R at probe time — confirming iCodon
                // is installed requires `library(iCodon)`, which is
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "iCodon requires R + the iCodon package; install R then \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = IcodonInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.icodon].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.icodon].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
        ...
        let staged_input_fasta: Option<String> = match input.input_fasta.as_ref() {
            Some(fasta_path) => {
                let source_fasta = confined_join(&case.path, fasta_path)?;
                if !source_fasta.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.icodon].input_fasta `{}` not found (resolved {})",
                            fasta_path.display(),
                ...
                let fasta_filename =
                    fasta_path
                        ...
                        .ok_or_else(|| AdapterError::InvalidCase {
                            case_path: case.path.join("case.toml"),
                            reason: format!(
                                "[bio.icodon].input_fasta path `{}` has no filename",
                                fasta_path.display()
                            ),
                        })?;
                ...
                Some(fasta_filename.to_string_lossy().to_string())
            }
            None => None,
        };

        // Drop a flat `valenx_params.json` into the workdir so the
        // user's R script can read the parsed `[bio.icodon]` knobs
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // iCodon predictions are typically fast (per-transcript
            // codon scoring against pre-trained regressors), but
            // genome-wide sweeps over tens of thousands of mRNAs
            // can run a while. 30 minutes covers most cases; the
            // user can re-run for larger batches.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting iCodon", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a
            // sentinel line `[valenx] icodon done` to signal
            // completion before exit; lift to a 95% progress tick.
            if line.contains("[valenx] icodon done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict typed outputs (.csv / .tsv / .rds / .png) to those
        // whose stem starts with the configured `output_basename`.
        // .log files are accepted regardless — R's `sink()` / system
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-icodon", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "iCodon stability table".to_string(),
                    });
                }
                Some("tsv") => {
                    if !stem_matches_basename {
                ...
                Some("rds") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "iCodon R object (RDS)".to_string(),
                    });
                }
                Some("png") => {
                    if !stem_matches_basename {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.icodon.predict"],
        }
    }
}
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = IcodonAdapter::new().info();
        assert_eq!(info.id, "icodon");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "GPL-3.0");
        assert_eq!(info.display_name, "iCodon");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = IcodonAdapter::new().info();
        // iCodon's GitHub releases haven't published versioned tags
        // consistently; the package self-reports 1.x. Upper bound 2.0
        // reserves room for a future major.
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = IcodonAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.icodon.predict"]);
    }
