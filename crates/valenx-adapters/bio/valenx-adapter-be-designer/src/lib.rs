//! # valenx-adapter-be-designer
//!
//! Adapter for [BE-Designer](http://www.rgenome.net/be-designer/) —
//! the rgenome.net base-editor guide design tool. Given a target DNA
//! window it enumerates sgRNAs whose editing window covers the
//! desired position for the supplied base editor (CBE / ABE family),
//! reporting on-target activity and bystander edit risk.
//!
//! **Phase 35.5 — Python subprocess wrapper for user-provided
//! scripts.** BE-Designer is a web tool (rgenome.net) backed by the
//! `bedesigner` Python package; the adapter doesn't contact the web
//! service. The user authors a `design.py` referenced from
//! `[bio.be-designer].script` in `case.toml` that does
//! `import bedesigner` (or shells out to the upstream pipeline) and
//! the actual design logic. `prepare()` stages the script (and an
//! optional input `.fa` template) into the workdir, drops a flat
...
//!
//! On `collect()` we walk the workdir for `<basename>*.csv` (guide
//! tables), `<basename>*.fasta` (designed sequence exports), plus any
...
use crate::case_input::BeDesignerInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(BeDesignerAdapter::new())
}

pub struct BeDesignerAdapter;

impl BeDesignerAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for BeDesignerAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "be-designer";
...
impl Adapter for BeDesignerAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "BE-Designer",
            // BE-Designer 1.x is the current rgenome.net line; 2.0
            ...
            tool_license: "MIT",
            docs_url: "http://www.rgenome.net/be-designer/",
            homepage_url: "http://www.rgenome.net/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import bedesigner` — confirms the `bedesigner`
                ...
                // can still validate.
                let import_ok = bedesigner_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `bedesigner` — install with `pip install bedesigner` \
                         (or the upstream rgenome distribution) for runs to \
                         succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with bedesigner installed; \
                       `pip install bedesigner` after ensuring python3 \
                       is on PATH"
                    .into(),
            }),
        }
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = BeDesignerInput::from_case_dir(&case.path)?;
        ...
        valenx_core::adapter_helpers::validate_output_basename(
            ...
            "[bio.be-designer].output_basename",
        )
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.be-designer].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.be-designer].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.be-designer].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.be-designer].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        // user's Python script can read the parsed `[bio.be-designer]`
        ...
        let binary_path =
            valenx_core::adapter_helpers::resolve_python_binary(&input.python, PYTHON_BINARIES)
                .map_err(|e| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!("[bio.be-designer].python: {e}"),
                })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // BE-Designer guide enumeration is CPU-light: a few
            // thousand sgRNA windows scored against PAM / editing-window
            // rules. 30 minutes is a generous default.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting be-designer", |line| {
            let mut hint = subprocess::Hint::default();
            if line.contains("[valenx] be-designer done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict typed outputs (.csv / .fasta) to those whose stem
        // starts with the configured `output_basename`. .log files
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-be-designer", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "BE-Designer guide table".to_string(),
                    });
                }
                Some("fasta") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "BE-Designer designed sequences".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "BE-Designer log".to_string(),
                    });
                }
                _ => continue,
            }
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.be_designer.design"],
        }
    }
}
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = BeDesignerAdapter::new().info();
        assert_eq!(info.id, "be-designer");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "BE-Designer");
    }
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = BeDesignerAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.be_designer.design"]);
    }
