//! # valenx-adapter-crispritz
//!
//! Adapter for [CRISPRitz](https://github.com/pinellolab/CRISPRitz) —
//! the Pinello lab off-target search tool (Cancellieri et al.,
//! Bioinformatics). CRISPRitz enumerates putative CRISPR off-target
//! sites in a reference genome, supporting mismatches, DNA / RNA
//! bulges, and PAM variants. Sister to Cas-OFFinder for the off-target
//! cohort within the CRISPR-design family.
//!
//! **Phase 35.6 — Python subprocess wrapper for user-provided
//! scripts.** CRISPRitz ships as the upstream `crispritz` Python
//! package (github.com/pinellolab/CRISPRitz); the adapter doesn't
//! reimplement search logic. The user authors a `search.py`
//! referenced from `[bio.crispritz].script` in `case.toml` that does
//! `import crispritz` and the actual search logic. `prepare()` stages
//! the script (and an optional input `.fa` template) into the
...
//! not supply one.
//!
//! On `collect()` we walk the workdir for `<basename>*.txt`
//! (off-target tables), `<basename>*.bed` (off-target BED files),
//! plus any `*.log` files Python emits.
...
use crate::case_input::CrispritzInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(CrispritzAdapter::new())
}

pub struct CrispritzAdapter;

impl CrispritzAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for CrispritzAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "crispritz";
const PYTHON_BINARIES: &[&str] = &["python3", "python"];

impl Adapter for CrispritzAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "CRISPRitz",
            // CRISPRitz 2.6 is the modern 2.x line we test against;
            // 3.0 reserves room for the next major.
            version_range: VersionRange {
                min_inclusive: Version::new(2, 6, 0),
                max_exclusive: Version::new(3, 0, 0),
            },
            physics: &[Physics::Bio],
            license_mode: LicenseMode::Subprocess,
            tool_license: "MIT",
            docs_url: "https://github.com/pinellolab/CRISPRitz",
            homepage_url: "https://github.com/pinellolab/CRISPRitz",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                let import_ok = crispritz_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `crispritz` — install from upstream \
                         https://github.com/pinellolab/CRISPRitz for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with crispritz installed; install \
                       from upstream \
                       https://github.com/pinellolab/CRISPRitz after \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = CrispritzInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.crispritz].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.crispritz].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.crispritz].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.crispritz].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // CRISPRitz off-target search runs over a reference genome;
            // single-guide jobs are minutes, full library scans on a
            // large genome can stretch to hours. 4 hours covers most
            // production scans.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting CRISPRitz", |line| {
            let mut hint = subprocess::Hint::default();
            if line.contains("[valenx] crispritz done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-crispritz", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("txt") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "CRISPRitz off-target table".to_string(),
                    });
                }
                Some("bed") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Tabular,
                        checksum: None,
                        label: "CRISPRitz off-target BED".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.crispritz.search"],
        }
    }
}
...
fn crispritz_importable(python_binary: &Path) -> bool {
    let output = std::process::Command::new(python_binary)
        .arg("-c")
        .arg("import crispritz")
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = CrispritzAdapter::new().info();
        assert_eq!(info.id, "crispritz");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "CRISPRitz");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = CrispritzAdapter::new().info();
        assert_eq!(info.version_range.min_inclusive, Version::new(2, 6, 0));
        assert_eq!(info.version_range.max_exclusive, Version::new(3, 0, 0));
    }

    #[test]
    fn capabilities_publishes_ribbon_contribution() {
        let caps = CrispritzAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.crispritz.search"]);
    }
