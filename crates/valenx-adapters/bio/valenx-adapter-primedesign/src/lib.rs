//! # valenx-adapter-primedesign
//!
//! Adapter for [PrimeDesign](https://primedesign.pinellolab.partners.org/) —
//! the Liu lab's prime-editing pegRNA / ngRNA design tool (Hsu et al.,
//! Nat Comms 2021). Given a target edit specified in HGVS notation it
//! enumerates pegRNA candidates with PBS / RTT lengths, scores them,
//! and emits the matched ngRNAs.
//!
//! **Phase 35.5 — Python subprocess wrapper for user-provided
//! scripts.** PrimeDesign ships as the upstream
//! `primedesign` Python package
//! (github.com/pinellolab/PrimeDesign); the adapter doesn't
//! reimplement design logic. The user authors a `design.py`
//! referenced from `[bio.primedesign].script` in `case.toml` that
//! does `import primedesign` and the actual design logic. `prepare()`
...
//! not supply one.
//!
//! On `collect()` we walk the workdir for `<basename>*.csv` (pegRNA
//! tables), `<basename>*.txt` (PrimeDesign report), plus any `*.log`
//! files Python emits.
...
use crate::case_input::PrimeDesignInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(PrimeDesignAdapter::new())
}

pub struct PrimeDesignAdapter;

impl PrimeDesignAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for PrimeDesignAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "primedesign";
const PYTHON_BINARIES: &[&str] = &["python3", "python"];

impl Adapter for PrimeDesignAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "PrimeDesign",
            // PrimeDesign 1.x is the current upstream line; 2.0
            ...
            tool_license: "MIT",
            docs_url: "https://github.com/pinellolab/PrimeDesign",
            homepage_url: "https://primedesign.pinellolab.partners.org/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                let import_ok = primedesign_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `primedesign` — install from upstream \
                         https://github.com/pinellolab/PrimeDesign for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with primedesign installed; install \
                       from upstream \
                       https://github.com/pinellolab/PrimeDesign after \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = PrimeDesignInput::from_case_dir(&case.path)?;
        ...
        valenx_core::adapter_helpers::validate_output_basename(
            ...
            "[bio.primedesign].output_basename",
        )
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.primedesign].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.primedesign].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.primedesign].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.primedesign].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // PrimeDesign pegRNA enumeration is CPU-light: window
            // scanning + Bio.Restriction-style scoring; 30 minutes
            // covers batch jobs over many target edits.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting primedesign", |line| {
            let mut hint = subprocess::Hint::default();
            if line.contains("[valenx] primedesign done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-primedesign", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "PrimeDesign pegRNA table".to_string(),
                    });
                }
                Some("txt") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Tabular,
                        checksum: None,
                        label: "PrimeDesign report".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "PrimeDesign log".to_string(),
                    });
                }
                _ => continue,
            }
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.primedesign.design"],
        }
    }
}
...
fn primedesign_importable(python_binary: &Path) -> bool {
    let output = std::process::Command::new(python_binary)
        .arg("-c")
        .arg("import primedesign")
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = PrimeDesignAdapter::new().info();
        assert_eq!(info.id, "primedesign");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "PrimeDesign");
    }
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = PrimeDesignAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.primedesign.design"]);
    }
