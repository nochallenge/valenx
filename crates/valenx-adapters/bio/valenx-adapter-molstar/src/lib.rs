//! # valenx-adapter-molstar
//!
//! Adapter for [Mol*](https://molstar.org/) — the modern WebGL-based
//! 3D molecular viewer behind PDBe and RCSB. The de-facto in-browser
//! successor to PyMOL / Chimera for quick web-shareable views of
//! structures, density maps, and trajectories. The core library is
//! JavaScript; this adapter wraps the `molstar` Python binding so a
//! user-authored `viewer.py` can build a Mol* state (`.molj`),
//! export a self-contained HTML viewer, or render a still PNG, all
//! from the standard subprocess pattern.
//!
//! **Phase 42 — Python subprocess wrapper for user-provided
//! scripts.** Mol* (via its Python binding) is a library, so the
//! adapter itself doesn't generate Python; the user authors a
//! `viewer.py` referenced from `[bio.molstar].script` in `case.toml`
//! that does `import molstar` and the actual viewer / render logic.
...
//! (self-contained viewer pages), `<basename>*.molj` (Mol* state
//! files), and `<basename>*.png` (rendered images), plus any `*.log`
//! files Python emits.
...
use crate::case_input::MolstarInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(MolstarAdapter::new())
}

pub struct MolstarAdapter;
...
impl Default for MolstarAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "molstar";
...
impl Adapter for MolstarAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "Mol*",
            // Mol* 3.x is the modern post-redesign line; 5.0 reserves
            // room for the next two majors of this actively maintained
            // viewer.
            ...
            tool_license: "MIT",
            docs_url: "https://molstar.org/viewer-docs/",
            homepage_url: "https://molstar.org/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import molstar` — confirms the `molstar` package
                ...
                let import_ok = molstar_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `molstar` — install with `pip install molstar` for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with the molstar Python binding \
                       installed; `pip install molstar` after ensuring \
                       python3 is on PATH"
                    .into(),
            }),
        }
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = MolstarInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.molstar].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.molstar].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(struct_path) => {
                ...
                if !source_struct.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.molstar].input_structure `{}` not found (resolved {})",
                            struct_path.display(),
                ...
                let struct_name =
                    struct_path
                        ...
                        .ok_or_else(|| AdapterError::InvalidCase {
                            case_path: case.path.join("case.toml"),
                            reason: format!(
                                "[bio.molstar].input_structure path `{}` has no filename",
                                struct_path.display()
                            ),
                        })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // Mol* viewer / render scripts are typically dominated by
            // structure parsing + a single render pass; PNG export
            // walks the GPU pipeline once. 30 minutes is a generous
            // default; multi-state movie exports might approach it.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting molstar", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a sentinel
            // line `[valenx] molstar done` to signal completion before
            // exit; lift to a 95% progress tick.
            if line.contains("[valenx] molstar done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict typed outputs (.html / .molj / .png) to those whose
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-molstar", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("html") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "Mol* viewer HTML".to_string(),
                    });
                }
                Some("molj") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Native,
                        checksum: None,
                        label: "Mol* state file".to_string(),
                    });
                }
                Some("png") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Native,
                        checksum: None,
                        label: "Mol* rendered image".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.molstar.view"],
        }
    }
}
...
/// Run `python -c "import molstar"` and return whether it succeeded.
/// Returns `false` on any failure (interpreter unusable, molstar not
/// importable); `probe()` lifts that to a "molstar not importable"
/// warning rather than a hard error so a missing-package case can
/// still validate.
fn molstar_importable(python_binary: &Path) -> bool {
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = MolstarAdapter::new().info();
        assert_eq!(info.id, "molstar");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "Mol*");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = MolstarAdapter::new().info();
        // Mol* 3.x is the modern line; 5.0 reserves room for the next
        // two majors of this actively maintained viewer.
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = MolstarAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.molstar.view"]);
    }
