//! # valenx-adapter-pegfinder
//!
//! Adapter for [pegFinder](http://pegfinder.sidichenlab.org/) — the
//! Komor lab pegRNA finder for prime editing (Chen et al., Nucleic
//! Acids Research). Given a target edit it enumerates pegRNA
//! candidates with PBS / RTT lengths, ranks them, and reports
//! off-target liability.
//!
//! **Phase 35.5 — Python subprocess wrapper for user-provided
//! scripts.** pegFinder ships as the upstream `pegfinder` Python
//! package (github.com/sychen9584/pegFinder); the adapter doesn't
//! reimplement design logic. The user authors a `design.py`
//! referenced from `[bio.pegfinder].script` in `case.toml` that
//! does `import pegfinder` and the actual design logic. `prepare()`
...
//! candidate tables), `<basename>*.txt` (summary), plus any `*.log`
//! files Python emits.
...
use crate::case_input::PegFinderInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(PegFinderAdapter::new())
}

pub struct PegFinderAdapter;

impl PegFinderAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for PegFinderAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "pegfinder";
const PYTHON_BINARIES: &[&str] = &["python3", "python"];

impl Adapter for PegFinderAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "pegFinder",
            // pegFinder 1.x is the current upstream line; 2.0
            ...
            tool_license: "MIT",
            docs_url: "https://github.com/sychen9584/pegFinder",
            homepage_url: "http://pegfinder.sidichenlab.org/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                let import_ok = pegfinder_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `pegfinder` — install from upstream \
                         https://github.com/sychen9584/pegFinder for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with pegfinder installed; install \
                       from upstream \
                       https://github.com/sychen9584/pegFinder after \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = PegFinderInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.pegfinder].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.pegfinder].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.pegfinder].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.pegfinder].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // pegFinder pegRNA enumeration is CPU-light: window
            // scanning + scoring; 30 minutes covers batch jobs
            // over many target edits.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting pegfinder", |line| {
            let mut hint = subprocess::Hint::default();
            if line.contains("[valenx] pegfinder done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-pegfinder", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "pegFinder pegRNA candidates".to_string(),
                    });
                }
                Some("txt") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Tabular,
                        checksum: None,
                        label: "pegFinder summary".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.pegfinder.design"],
        }
    }
}
...
fn pegfinder_importable(python_binary: &Path) -> bool {
    let output = std::process::Command::new(python_binary)
        .arg("-c")
        .arg("import pegfinder")
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = PegFinderAdapter::new().info();
        assert_eq!(info.id, "pegfinder");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "pegFinder");
    }
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = PegFinderAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.pegfinder.design"]);
    }
