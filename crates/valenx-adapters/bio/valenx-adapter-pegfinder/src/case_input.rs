//! `[bio.pegfinder]` case-input parsing. Schema:
//!
//! ```toml
//! [case]
//! physics = "bio"
//! solver  = "pegfinder.design"
//!
//! [bio.pegfinder]
//! script           = "design.py"
...
//! ```
//!
//! pegFinder is the Komor lab pegRNA finder for prime editing
//! (Chen et al., Nucleic Acids Research): given a target edit it
//! enumerates pegRNA candidates with PBS / RTT lengths, ranks them,
//! and reports off-target liability. The adapter itself doesn't
//! contact the web app at pegfinder.sidichenlab.org; the user authors
//! a `design.py` that imports the upstream `pegfinder` Python package
//! (github.com/sychen9584/pegFinder) and we spawn `python <script>`
...
pub struct PegFinderInput {
    ...
    /// `<basename>*.csv` (pegRNA candidates), `<basename>*.txt`
...
impl PegFinderInput {
    pub fn from_case_dir(case_dir: &std::path::Path) -> Result<Self, AdapterError> {
        ...
        let block = parsed
            .get("bio")
            .and_then(|v| v.get("pegfinder"))
            .ok_or_else(|| {
                AdapterError::Other(anyhow::anyhow!(
                    "{} missing [bio.pegfinder] section",
                    case_toml.display()
                ))
            })?;
        ...
        let script = block
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                AdapterError::Other(anyhow::anyhow!("[bio.pegfinder].script required"))
            })?;
        if script.is_empty() {
            return Err(AdapterError::Other(anyhow::anyhow!(
                "[bio.pegfinder].script must not be empty"
            )));
        }
        ...
        if !ext_ok {
            return Err(AdapterError::Other(anyhow::anyhow!(
                "[bio.pegfinder].script `{script}` must end in `.py`"
            )));
        }
        ...
        let output_basename = block
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                AdapterError::Other(anyhow::anyhow!("[bio.pegfinder].output_basename required"))
            })?;
        if output_basename.is_empty() {
            return Err(AdapterError::Other(anyhow::anyhow!(
                "[bio.pegfinder].output_basename must not be empty"
            )));
        }
...
"#,
        ...
        .unwrap();
        let input = PegFinderInput::from_case_dir(&d).unwrap();
    ...
    fn parses_with_input_fasta() {
        ...
        // enumerates pegRNA candidates over.
        let d = tempdir("pegfinder-input");
        std::fs::write(
            d.join("case.toml"),
            r#"[case]
physics = "bio"
...
[bio.pegfinder]
script          = "design.py"
python          = "/opt/conda/envs/pegfinder/bin/python"
input_fasta     = "target.fa"
output_basename = "candidates"
"#,
        )
        .unwrap();
        let input = PegFinderInput::from_case_dir(&d).unwrap();
        assert_eq!(input.python, "/opt/conda/envs/pegfinder/bin/python");
        assert_eq!(input.input_fasta, Some(PathBuf::from("target.fa")));
        assert_eq!(input.output_basename, "candidates");
        let _ = std::fs::remove_dir_all(&d);
    }

    #[test]
    fn rejects_non_py_extension() {
        let d = tempdir("pegfinder-badext");
        std::fs::write(
            d.join("case.toml"),
            r#"[case]
physics = "bio"
...
"#,
        ...
        .unwrap();
        let err = PegFinderInput::from_case_dir(&d).unwrap_err();
        let msg = format!("{err}");
