//! # valenx-adapter-pysbol
//!
//! Adapter for [pySBOL3](https://pysbol3.readthedocs.io/) — the
//! Python implementation of the [Synthetic Biology Open Language
//! (SBOL)](https://sbolstandard.org/), the canonical data model
//! for genetic-circuit designs. SBOL captures components,
//! sequences, interactions, constraints, and the full provenance
//! of a synthetic design as RDF/XML or JSON-LD that round-trips
//! with every SBOL-conformant tool: j5, Cello, SynBioHub,
//! iBioSim, ...
//!
//! **Phase 33 — subprocess wrapper for user-provided pySBOL3
//! scripts.** The user authors a `compose.py` referenced from
//! `[bio.pysbol].script` in `case.toml`, optionally an existing
//! SBOL document to extend, and an output basename. `prepare()`
//! stages the script + optional input SBOL into the workdir and
//! writes a flat `valenx_params.json` so the script can read the
...
//! ```
//!
//! `input_sbol` is `null` when the user omitted the optional
//! starting document. Scripts read with
//! `json.load(open("valenx_params.json"))` and resolve the
//! filename relative to the cwd (the workdir).
//!
//! On `collect()` we walk the workdir for `<basename>*.xml`
//! (the SBOL document the script wrote) and `<basename>*.json`
//! (composition logs / metadata).
...
use crate::case_input::PySbolInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(PySbolAdapter::new())
}

pub struct PySbolAdapter;
...
impl Default for PySbolAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "pysbol";
...
impl Adapter for PySbolAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "pySBOL",
            // pySBOL3 is the third-generation Python binding; the
            // 3.x line ships the modern SBOL3 data model and is the
            // canonical choice for new projects. Upper bound 4.0
            ...
            docs_url: "https://pysbol3.readthedocs.io/",
            homepage_url: "https://sbolstandard.org/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import sbol3` to confirm the library is
                ...
                let importable = sbol3_importable(&binary_path);
                let mut warnings = Vec::new();
                if !importable {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `sbol3` — install with `pip install sbol3` for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.9+ with pySBOL3 installed; `pip install sbol3` \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = PySbolInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.pysbol].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.pysbol].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
        ...
        // Optionally stage the input SBOL document so the script can
        // resolve it via a bare filename inside the workdir.
        let staged_input_sbol: Option<String> = match input.input_sbol.as_ref() {
            Some(sbol_path) => {
                let source_sbol = confined_join(&case.path, sbol_path)?;
                if !source_sbol.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.pysbol].input_sbol `{}` not found (resolved {})",
                            sbol_path.display(),
                            source_sbol.display()
                        ),
                    });
                ...
                let sbol_filename =
                    sbol_path
                        ...
                        .ok_or_else(|| AdapterError::InvalidCase {
                            case_path: case.path.join("case.toml"),
                            reason: format!(
                                "[bio.pysbol].input_sbol path `{}` has no filename",
                                sbol_path.display()
                            ),
                        })?;
                let dest_sbol = workdir.join(sbol_filename);
                if source_sbol != dest_sbol {
                    fs::copy(&source_sbol, &dest_sbol)?;
                }
                Some(sbol_filename.to_string_lossy().to_string())
            }
            None => None,
        };

        // Drop a flat `valenx_params.json` into the workdir so the
        // user's script can read the parsed `[bio.pysbol]` knobs
        // without having to reparse case.toml itself. Built by
        // hand to avoid pulling in serde_json for a 2-key flat
        // object. `input_sbol` is the staged filename (or `null` if
        // the user opted out).
        let input_sbol_json = match staged_input_sbol.as_deref() {
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // SBOL composition is a near-instant operation —
            // building / validating an in-memory document is bound
            // by Python interpreter spin-up, not by the SBOL math.
            // 30 minutes covers very large designs with extensive
            // validation passes.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting pySBOL", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a
            // sentinel line `[valenx] pysbol done` to signal
            // completion before exit; lift to a 95% progress tick.
            if line.contains("[valenx] pysbol done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict XML / JSON outputs to those whose stem starts
        // with the configured `output_basename`. JSON files outside
        // that prefix (e.g. `valenx_params.json`) are intentionally
        // skipped. Failure to read params is non-fatal — accept
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-pysbol", ?e, "workdir read failed");
                return Ok(results);
            }
        };
        for entry in entries.flatten() {
            ...
            let stem = path
                ...
                .unwrap_or("")
                .to_string();
            // For XML, restrict to outputs that match the
            // configured basename prefix; for JSON, only accept
            // basename-prefixed files (so `valenx_params.json`
            // doesn't leak into the artefact list).
            match ext.as_deref() {
                Some("xml") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "pySBOL document".to_string(),
                    });
                }
                Some("json") => {
                    let stem_ok = match basename.as_deref() {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.pysbol.compose"],
        }
    }
}

/// Probe whether `import sbol3` succeeds in the given Python
/// interpreter. Returns true only when Python exits 0.
fn sbol3_importable(python_binary: &Path) -> bool {
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = PySbolAdapter::new().info();
        assert_eq!(info.id, "pysbol");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "Apache-2.0");
        assert_eq!(info.display_name, "pySBOL");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = PySbolAdapter::new().info();
        // pySBOL3 is the third-generation Python binding; the 3.x
        // line ships the modern SBOL3 data model. Upper bound 4.0
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = PySbolAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.pysbol.compose"]);
    }
