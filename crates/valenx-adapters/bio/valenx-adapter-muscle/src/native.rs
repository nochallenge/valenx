//! Native Rust MSA path for the MUSCLE adapter.
//!
//! Uses `valenx_align::msa`: progressive guide-tree alignment followed
//! by MUSCLE-class iterative refinement (partition-and-realign until no
//! sum-of-pairs improvement). This implements the same algorithmic class
//! as MUSCLE (and MAFFT, ClustalΩ) — progressive + iterative — using
//! the same substitution matrices (BLOSUM62 for protein, NUC.4.4 for
//! nucleotide) and affine gap costs.
//!
...
pub struct NativeMsaParams {
    ...
    /// Defaults to `aligned.fa` for MUSCLE/MAFFT; ClustalΩ uses the
    /// configured basename + extension.
