//! # valenx-adapter-dnachisel
//!
//! Adapter for [DNA Chisel](https://edinburgh-genome-foundry.github.io/DnaChisel/)
//! — Edinburgh Genome Foundry's Python codon-optimization /
//! sequence-design library. DNA Chisel solves the constraint-
//! satisfaction problem of choosing codons that satisfy host codon
//! usage, GC-content windows, restriction-site avoidance,
//! homopolymer caps, and arbitrary user-defined constraints, all
//! while preserving the encoded protein sequence.
//!
//! **Phase 43 — Python subprocess wrapper for user-provided
//! scripts.** DNA Chisel is a library (not a CLI), so the adapter
//! itself doesn't generate Python; the user authors an `optimize.py`
//! referenced from `[bio.dnachisel].script` in `case.toml` that does
//! `from dnachisel import DnaOptimizationProblem` (or similar) and
//! the actual constraint setup. `prepare()` stages the script (and
//! an optional input `.fa` / `.fasta` template) into the workdir,
...
//!
//! On `collect()` we walk the workdir for `<basename>*.fasta`
//! (optimized FASTA), `<basename>*.gb` / `<basename>*.genbank`
//! (annotated GenBank), `<basename>*.json` (constraint reports),
//! `<basename>*.png` (objective-curve plots), plus any `*.log` files
//! Python emits.
...
use crate::case_input::DnaChiselInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(DnaChiselAdapter::new())
}

pub struct DnaChiselAdapter;

impl DnaChiselAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for DnaChiselAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "dnachisel";
...
impl Adapter for DnaChiselAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "DNA Chisel",
            // DNA Chisel 3.x is the current line — Python 3.7+,
            // post-Bio.Restriction-deprecation API. 4.0 reserves
            // room for the next major of this actively maintained
            // library.
            ...
            tool_license: "MIT",
            docs_url: "https://edinburgh-genome-foundry.github.io/DnaChisel/",
            homepage_url: "https://github.com/Edinburgh-Genome-Foundry/DnaChisel",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import dnachisel` — confirms the `dnachisel`
                ...
                // can still validate.
                let import_ok = dnachisel_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `dnachisel` — install with `pip install dnachisel` \
                         for runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.7+ with dnachisel installed; \
                       `pip install dnachisel` after ensuring python3 \
                       is on PATH"
                    .into(),
            }),
        }
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = DnaChiselInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.dnachisel].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.dnachisel].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
        ...
        // Optionally stage the input FASTA file so the script can
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.dnachisel].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.dnachisel].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // DNA Chisel optimization is constraint-solver-bound:
            // small problems (single short ORF) finish in seconds,
            // but full operon designs with many simultaneous
            // constraints can take many minutes. 30 minutes is a
            // generous default.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting dnachisel", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a
            // sentinel line `[valenx] dnachisel done` to signal
            // completion before exit; lift to a 95% progress tick.
            if line.contains("[valenx] dnachisel done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict typed outputs (.fasta / .gb / .genbank / .json /
        // .png) to those whose stem starts with the configured
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-dnachisel", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("fasta") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "DNA Chisel optimized FASTA".to_string(),
                    });
                }
                Some("gb") | Some("genbank") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Native,
                        checksum: None,
                        label: "DNA Chisel GenBank".to_string(),
                    });
                }
                Some("json") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "DNA Chisel constraint report".to_string(),
                    });
                }
                Some("png") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Native,
                        checksum: None,
                        label: "DNA Chisel plot".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "DNA Chisel log".to_string(),
                    });
                }
                _ => continue,
            }
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.dnachisel.optimize"],
        }
    }
}
...
/// Run `python -c "import dnachisel"` and return whether it
/// succeeded. Returns `false` on any failure (interpreter unusable,
/// dnachisel not importable); `probe()` lifts that to a "dnachisel
/// not importable" warning rather than a hard error so a missing-
/// package case can still validate.
fn dnachisel_importable(python_binary: &Path) -> bool {
    let output = std::process::Command::new(python_binary)
        .arg("-c")
        .arg("import dnachisel")
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = DnaChiselAdapter::new().info();
        assert_eq!(info.id, "dnachisel");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "DNA Chisel");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = DnaChiselAdapter::new().info();
        // DNA Chisel 3.x is the current line; 4.0 reserves room for
        // the next major of this actively maintained library.
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = DnaChiselAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.dnachisel.optimize"]);
    }
