//! # valenx-adapter-hoomd
//!
//! Adapter for [HOOMD-blue](https://hoomd-blue.readthedocs.io/) — the
//! Glotzer lab's GPU-native particle molecular dynamics engine
//! optimised for soft-matter, colloidal, and self-assembly
//! simulations.
//!
//! **Phase 5.6 — Python subprocess wrapper for user-provided
//! scripts.** From v3 onward HOOMD-blue ships no native CLI: every
...
//! itself doesn't generate Python; the user supplies a `production.py`
//! referenced from `[bio.hoomd].script` in `case.toml`. `prepare()`
//! stages the script into the workdir, drops a flat
...
//! ```
//!
//! Scripts read with `json.load(open("valenx_params.json"))` and use
//! the basename to name their GSD trajectory and any auxiliary HDF5
//! outputs (so `collect()` can find them).
//!
//! On `collect()` we walk the workdir for `<basename>*.gsd` (HOOMD
//! trajectories) and `<basename>*.h5` (HDF5 outputs), plus any
...
use crate::case_input::HoomdInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(HoomdAdapter::new())
}

pub struct HoomdAdapter;
...
impl Default for HoomdAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "hoomd";
...
impl Adapter for HoomdAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "HOOMD-blue",
            // HOOMD-blue 3.0 (Mar 2022) was the rewrite that ditched
            // the legacy command-API in favour of pure Python; v4 / v5
            // are incremental on top. 6.0 is reserved for a hypothetical
            // future major.
            ...
            homepage_url: "https://glotzerlab.engin.umich.edu/hoomd-blue/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import hoomd` — confirms the `hoomd` package is
                // importable from the chosen interpreter (vs. just
                ...
                let import_ok = hoomd_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `hoomd` — install HOOMD-blue with `conda install -c \
                         conda-forge hoomd` (the recommended channel ships \
                         GPU-enabled builds) for runs to succeed"
                            .into(),
                    );
                }
                Ok(ProbeReport {
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.9+ with HOOMD-blue installed; \
                       `conda install -c conda-forge hoomd` after \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = HoomdInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.hoomd].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.hoomd].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
        ...
        // user's Python script can read the parsed `[bio.hoomd]`
        // knobs without having to reparse case.toml itself. Built by
        // hand to avoid pulling in serde_json for a 1-key flat
        // object.
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // HOOMD-blue runs are GPU-bound and span a wide spectrum:
            // a small LJ-fluid validation finishes in seconds while a
            // production self-assembly trajectory can run for days.
            // 4 hours is a generous default for typical
            // soft-matter / colloidal cases.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting HOOMD-blue", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a
            // sentinel line `[valenx] hoomd done` to signal
            // completion before exit; lift to a 95% progress tick.
            if line.contains("[valenx] hoomd done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // Restrict typed outputs (.gsd / .h5) to those whose stem
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-hoomd", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("gsd") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "HOOMD trajectory (GSD)".to_string(),
                    });
                }
                Some("h5") => {
                    if !stem_matches_basename {
                    ...
                    artefacts.push(Artifact {
                        ...
                        kind: ArtifactKind::Native,
                        checksum: None,
                        label: "HOOMD HDF5 output".to_string(),
                    });
                }
                Some("log") => {
                    artefacts.push(Artifact {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.hoomd.simulate"],
        }
    }
}
...
/// Run `python -c "import hoomd"` and return whether it succeeded.
/// Returns `false` on any failure (interpreter unusable, hoomd not
/// importable); `probe()` lifts that to a "hoomd not importable"
/// warning rather than a hard error so a missing-package case can
/// still validate.
fn hoomd_importable(python_binary: &Path) -> bool {
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = HoomdAdapter::new().info();
        assert_eq!(info.id, "hoomd");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "BSD-3-Clause");
        assert_eq!(info.display_name, "HOOMD-blue");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = HoomdAdapter::new().info();
        // HOOMD-blue 3.0 (Mar 2022) was the rewrite that ditched the
        // legacy command-API in favour of pure Python; v4 / v5 are
        // incremental on top. 6.0 reserves room for a hypothetical
        // future major.
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = HoomdAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.hoomd.simulate"]);
    }
