//! # valenx-adapter-tskit
//!
//! Adapter for [tskit](https://tskit.dev/tskit/) — the canonical
//! tree-sequence analysis library, built around the succinct
//! tree-sequence data structure pioneered by msprime. tskit
//! computes population-genetics statistics (pi, Tajima's D, Fst,
//! site-frequency spectra, IBD shares), exposes per-tree iteration
//! across the genome, converts between tree-sequence and
//! VCF / Newick / table formats, and renders phylogenetic plots.
//! It's the workhorse downstream of every Phase 29 simulator —
//! msprime emits `.trees`, SLiM emits `.trees`, tskit consumes
//! them.
//!
//! **Phase 29 — subprocess wrapper for user-provided tskit
//! scripts.** The user authors an `analyze.py` referenced from
//! `[bio.tskit].script` in `case.toml`, plus the input tree
//! sequence (`input_trees`) and an output basename. `prepare()`
//! stages the script + tree-sequence file into the workdir under
//! their original filenames so the script can resolve them via
//! relative paths, then writes a flat `valenx_params.json` so the
//! script knows where to find them. `run()` invokes
...
//! and pass the values through to tskit themselves. This keeps the
//! adapter free of upstream API churn.
//!
//! On `collect()` we walk the workdir for `<basename>*.csv` /
//! `<basename>*.tsv` (statistics tables) and `*.png` (rendered
//! plots).
...
use crate::case_input::TskitInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(TskitAdapter::new())
}

pub struct TskitAdapter;
...
impl Default for TskitAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "tskit";
...
impl Adapter for TskitAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "tskit",
            // tskit 0.5+ ships the modern `Statistics` API surface
            // and the v3 tree-sequence file format that msprime
            // 1.3+ writes. Upper bound 1.0 reserves room for the
            // long-promised 1.0 release.
            version_range: VersionRange {
                min_inclusive: Version::new(0, 5, 0),
                max_exclusive: Version::new(1, 0, 0),
            },
            physics: &[Physics::Bio],
            license_mode: LicenseMode::Subprocess,
            tool_license: "MIT",
            docs_url: "https://tskit.dev/tskit/",
            homepage_url: "https://tskit.dev/tskit/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                // Try `import tskit` to confirm the library is
                ...
                let importable = tskit_importable(&binary_path);
                let mut warnings = Vec::new();
                if !importable {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `tskit` — install with `pip install tskit` (or \
                         `conda install -c conda-forge tskit`) for runs to \
                         succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.9+ with tskit installed; `pip install tskit` \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = TskitInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.tskit].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.tskit].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
        ...
        // Stage the input `.trees` tree-sequence file. Reading
        // from inside the workdir lets the script use the bare
        // filename rather than juggle absolute paths.
        let source_trees = confined_join(&case.path, &input.input_trees)?;
        if !source_trees.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.tskit].input_trees `{}` not found (resolved {})",
                    input.input_trees.display(),
                    source_trees.display()
                ),
            });
        }
        let trees_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.tskit].input_trees path `{}` has no filename",
                        input.input_trees.display()
                    ),
                })?;
        let dest_trees = workdir.join(trees_filename);
        if source_trees != dest_trees {
            fs::copy(&source_trees, &dest_trees)?;
        }

        // Drop a flat `valenx_params.json` into the workdir so the
        // user's script can read the parsed `[bio.tskit]` knobs
        // without having to reparse case.toml itself. Built by
        // hand to avoid pulling in serde_json for a 2-key flat
        // object. `input_trees` is the staged filename so the
        // script can open it via a relative path inside the
        // workdir.
        let params_json = format!(
            "{{\n  \"input_trees\": {},\n  \"output_basename\": {}\n}}\n",
            json_string(&trees_filename.to_string_lossy()),
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // Statistics on a typical tree sequence finish in
            // seconds-to-minutes; whole-genome IBD computation
            // can run for an hour or more. 2 hours is a generous
            // default.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting tskit", |line| {
            let mut hint = subprocess::Hint::default();
            // Convention: the user-supplied script can emit a
            // sentinel line `[valenx] tskit done` to signal
            // completion before exit; lift to a 95% progress tick.
            if line.contains("[valenx] tskit done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        // collected statistics tables to those whose stem starts
        // with the configured `output_basename`. Failure to read
        // the params is non-fatal — accept all `.csv` / `.tsv` in
        // that case.
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-tskit", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") | Some("tsv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "tskit analysis output".to_string(),
                    });
                }
                Some("png") => {
                    artefacts.push(Artifact {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.tskit.analyze"],
        }
    }
}

/// Probe whether `import tskit` succeeds in the given Python
...
fn tskit_importable(python_binary: &Path) -> bool {
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = TskitAdapter::new().info();
        assert_eq!(info.id, "tskit");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "tskit");
    }

    #[test]
    fn info_version_range_matches_supported_band() {
        let info = TskitAdapter::new().info();
        // tskit 0.5+ ships the modern Statistics API surface and
        // the v3 tree-sequence file format; upper bound 1.0
        // reserves room for the long-promised 1.0 release.
        assert_eq!(info.version_range.min_inclusive, Version::new(0, 5, 0));
        assert_eq!(info.version_range.max_exclusive, Version::new(1, 0, 0));
    }

    #[test]
    fn capabilities_publishes_ribbon_contribution() {
        let caps = TskitAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.tskit.analyze"]);
    }
