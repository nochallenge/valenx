//! Native Rust MSA path for the Clustal Omega adapter.
//!
...
//! improvement). This implements the same algorithmic class as ClustalΩ,
//! MAFFT, and MUSCLE — progressive + iterative — using BLOSUM62 for
//! protein and NUC.4.4 for nucleotide, with affine gap costs.
//!
//! The native path always emits FASTA output regardless of the
//! `outfmt` field in the case. Users who explicitly need Clustal or
//! Stockholm format should install the `clustalo` binary.
...
pub struct NativeMsaParams {
    ...
    /// For ClustalΩ this is `<output_basename>.<ext>` where `ext`
    /// matches the requested outfmt.
...
/// Command sentinel: run native MSA, don't spawn a child process.
...
/// writing aligned FASTA to `workdir/<params.output_name>`.
...
///
/// Heuristic: if >80% of non-gap residues in the first 1000 bytes per
...
mod tests {
    ...
    fn write_and_read_params_round_trips() {
        let d = tempdir("clustalo-native");
        let params = NativeMsaParams {
            input_path: "/tmp/seqs.fa".to_string(),
            output_name: "alignment.fasta".to_string(),
            refine: true,
            max_iterations: 8,
        };
