//! # valenx-adapter-indelphi
//!
//! Adapter for [inDelphi](https://github.com/maxwshen/inDelphi-model) —
//! Max Shen / David Liu lab's Cas9-cut indel pattern predictor
//! (Shen et al., Nature). Given a target window and a guide RNA it
//! predicts the indel-frequency distribution that will result from
//! NHEJ repair after a Cas9 double-strand break, ranking +1 insertions
//! and microhomology-mediated deletions by predicted frequency.
//!
//! **Phase 35.6 — Python subprocess wrapper for user-provided
//! scripts.** inDelphi ships as the upstream `inDelphi` Python package
//! (github.com/maxwshen/inDelphi-model); the adapter doesn't
//! reimplement prediction logic. The user authors a `predict.py`
//! referenced from `[bio.indelphi].script` in `case.toml` that does
//! `import inDelphi` and the actual prediction logic. `prepare()`
...
//! indel tables), `<basename>*.png` (visualisation plots), plus any
...
use crate::case_input::IndelphiInput;

pub fn adapter() -> Box<dyn Adapter> {
    Box::new(IndelphiAdapter::new())
}

pub struct IndelphiAdapter;

impl IndelphiAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl Default for IndelphiAdapter {
    fn default() -> Self {
        Self::new()
    }
}

const INFO_ID: &str = "indelphi";
const PYTHON_BINARIES: &[&str] = &["python3", "python"];

impl Adapter for IndelphiAdapter {
    fn info(&self) -> AdapterInfo {
        AdapterInfo {
            id: INFO_ID,
            display_name: "inDelphi",
            // inDelphi 1.x is the current upstream line; 2.0 reserves
            ...
            tool_license: "MIT",
            docs_url: "https://github.com/maxwshen/inDelphi-model",
            homepage_url: "https://indelphi.giffordlab.mit.edu/",
        }
    }
    ...
    fn probe(&self) -> Result<ProbeReport, AdapterError> {
        match find_on_path(PYTHON_BINARIES) {
            Some(binary_path) => {
                let import_ok = indelphi_importable(&binary_path);
                let mut warnings = Vec::new();
                if !import_ok {
                    warnings.push(
                        "probe found `python` on PATH but could not import \
                         `inDelphi` — install from upstream \
                         https://github.com/maxwshen/inDelphi-model for \
                         runs to succeed"
                            .into(),
                    );
                }
            ...
            None => Err(AdapterError::ToolNotInstalled {
                name: INFO_ID,
                hint: "Python 3.8+ with inDelphi installed; install \
                       from upstream \
                       https://github.com/maxwshen/inDelphi-model after \
    ...
    fn prepare(&self, case: &Case, workdir: &Path) -> Result<PreparedJob, AdapterError> {
        let input = IndelphiInput::from_case_dir(&case.path)?;
        ...
        if !source_script.is_file() {
            return Err(AdapterError::InvalidCase {
                case_path: case.path.join("case.toml"),
                reason: format!(
                    "[bio.indelphi].script `{}` not found (resolved {})",
        ...
        let script_filename =
            input
                ...
                .ok_or_else(|| AdapterError::InvalidCase {
                    case_path: case.path.join("case.toml"),
                    reason: format!(
                        "[bio.indelphi].script path `{}` has no filename",
                        input.script.display()
                    ),
                })?;
            ...
            Some(fa_path) => {
                ...
                if !source_fa.is_file() {
                    return Err(AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.indelphi].input_fasta `{}` not found (resolved {})",
                            fa_path.display(),
                            source_fa.display()
                        ),
                    });
                ...
                let fa_name = fa_path
                    ...
                    .ok_or_else(|| AdapterError::InvalidCase {
                        case_path: case.path.join("case.toml"),
                        reason: format!(
                            "[bio.indelphi].input_fasta path `{}` has no filename",
                            fa_path.display()
                        ),
                    })?;
        ...
        Ok(PreparedJob {
            ...
            native_command,
            environment: Vec::new(),
            // inDelphi prediction is CPU-light: a small ML model
            // evaluating per-window indel distributions; 30 minutes
            // covers batch jobs over many target sites.
    ...
    fn run(&self, job: &PreparedJob, ctx: &mut RunContext) -> Result<RunReport, AdapterError> {
        let report = subprocess::run(job, ctx, "starting inDelphi", |line| {
            let mut hint = subprocess::Hint::default();
            if line.contains("[valenx] inDelphi done") {
    ...
    fn collect(&self, job: &PreparedJob) -> Result<Results, AdapterError> {
        ...
        let entries = match fs::read_dir(&job.workdir) {
            Ok(e) => e,
            Err(e) => {
                tracing::warn!(target: "valenx-indelphi", ?e, "workdir read failed");
                return Ok(results);
            }
        };
            ...
            match ext.as_deref() {
                Some("csv") => {
                    ...
                    artefacts.push(Artifact {
                        ...
                        checksum: None,
                        label: "inDelphi indel predictions".to_string(),
                    });
                }
                Some("png") => {
                    if !stem_matches_basename {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities {
            ...
            ribbon_contributions: vec!["bio.indelphi.predict"],
        }
    }
}
...
fn indelphi_importable(python_binary: &Path) -> bool {
...
mod tests {
    ...
    fn info_is_bio_domain() {
        let info = IndelphiAdapter::new().info();
        assert_eq!(info.id, "indelphi");
        assert_eq!(info.physics, &[Physics::Bio]);
        assert_eq!(info.tool_license, "MIT");
        assert_eq!(info.display_name, "inDelphi");
    }
    ...
    fn capabilities_publishes_ribbon_contribution() {
        let caps = IndelphiAdapter::new().capabilities();
        assert!(caps.capabilities.is_empty());
        assert_eq!(caps.ribbon_contributions, vec!["bio.indelphi.predict"]);
    }
