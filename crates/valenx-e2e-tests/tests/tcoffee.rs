//! E2E integration test for the T-Coffee adapter.
//!
//! Spawns a real `t_coffee` subprocess via the adapter's
//! prepare/run/collect pipeline. Skipped automatically when the
//! upstream binary isn't on PATH.

use valenx_adapter_tcoffee::TCoffeeAdapter;
...
#[test]
fn tcoffee_runs_end_to_end_against_real_binary() {
    let adapter = TCoffeeAdapter::new();
    if skip_if_missing(&adapter) {
        return;
    }

    let case_dir = tempdir("e2e-tcoffee");
    ...
    assert_eq!(report.exit_code, 0, "t_coffee exited non-zero: {report:?}");
